...
$(document).ready(function () {
    if(window.location.href.indexOf("index.php") != -1 && window.location.href.indexOf("index.php?") == -1 ) {
		...
		username_text = $("div.bloc table tbody tr:first td:first").html().replace(':','');
		pass_text = $("div.bloc > form > table > tbody > tr:nth-child(2) > td:first").html().replace(':','');
...
//DATE(TOP-RIGHT corner) & Side menu Stuff
...
//GOT HELP//
          ...
          $('<li class="copied"></li>').html($(this).children('a').clone()).
                prependTo($(this).children('ul')).
                find('> a > span').html('Overview');
    });
